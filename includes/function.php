...
function checkSignSalt($data_info){
    ...
    $data_arr = json_decode_nice(urldecode(base64_decode($data_json)),true);

    if($data_arr['sign'] == '' && $data_arr['salt'] == '' ){
//$data['data'] = array("status" => -1, "message" => "Invalid sign salt.");

        $set['ALL_IN_ONE_VIDEO'][] = array("status" => -1, "message" => "Invalid sign salt.");
...
{ 
    // Bypass for testing - return hardcoded data
    $fake_data = new stdClass();
    $fake_data->buyer = 'buyer';
    $fake_data->item = new stdClass();
    $fake_data->item->id = '9012163';
    return $fake_data;
