...
$sql = "SELECT * FROM tbl_video LEFT JOIN tbl_category ON tbl_video.`cat_id`=tbl_category.`cid` WHERE tbl_video.`status`='1' AND tbl_video.`id`='$id'";

$res = mysqli_query($mysqli, $sql);
$row = mysqli_fetch_assoc($res);

$sql1 = "SELECT tbl_reports.*, tbl_users.`name` FROM tbl_reports, tbl_users WHERE tbl_reports.`post_id`='$id' AND tbl_users.`id`=tbl_reports.`user_id` ORDER BY tbl_reports.`report_on` DESC";
				...
				<li class="message">
					<a href="javascript:void(0)">
						<div class="message">
							...
							<div class="content">
								<div class="title">&nbsp;&nbsp;<span class="total_reports"><?= total_reports($id) ?></span> Comments</div>
							</div>
						</div>
					</a>
				</li>
						...
						?>
							<li class="<?= $value['id'] ?>" style="padding-right: 20px">
								...
								<div class="info" style="clear: both;">
									<div class="datetime">
										<?= calculate_time_span($value['report_on'], true) ?>
										<a href="" class="btn_delete_report" data-id="<?= $value['id'] ?>" style="color: red;text-decoration: none;"><i class="fa fa-trash"></i> Delete</a>
									</div>
								</div>
							</li>
					<?php }
